...
export function showMethodInfo(mPtr: NativePointer | Il2Cpp.Method): void {
    ...
    LOGZ(`${getLine(4, ' ')}[-]${packMethod.class.name} @ ${packMethod.class.handle} | M:${packMethod.class.methods.length} | F:${packMethod.class.fields.length}${packMethod.class.namespace?.length ? ` | N:${packMethod.class.namespace}` : ''}`)
